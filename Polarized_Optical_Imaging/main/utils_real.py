...
def createSaveFolders(root_path_object, path_views):
    for v in range(len(path_views)):
        ...
        xmkdir(path_npy)
        xmkdir(path_video)
        folders_list = ['img_diffuse','img_residue','img_single','img_single_1',
                        'map', 'render_shading',
        'render_specular','render_sscatter','render_subscatter','render_diffuse',
        'render_diffimage','render_diffimage_other','render_singleimage','render_singleimage_other']
...
def unlitShadingRender(map_normal, map_mask, lightDirs,diffuseIntensity,weight_shading):
    ...
    render_unlit_shading = render_unlit_shading *diffuseIntensity* unlit_mask
...
def singleShadingRender(map_normal, map_mask, lightDir,diffuseIntensity,weight_shading):
    ...
    return render_shading *diffuseIntensity* map_mask
...
def singleSScatterRender(map_mask,map_normal,map_sscatter,k1,k2,render_rho_dt_L,render_rho_dt_V,render_shading,weight_lambert, weight_shading):
    ...
    return render_sscatter * map_mask
...
def unlitSubScatterRender(k2,diffuse_back,map_diffuse,map_specular,map_normal,map_mask,rho_dt,render_unlit_mix_irradiance,ambientIntensity):
    ...
    map_mask_num = np.tile(np.expand_dims(map_mask,0), reps=(num_lights, 1, 1, 1))
    ...
    render_unlit_subscatter = render_unlit_subscatter_out * map_mask_num * (1-ambientIntensity)
    # 考虑全局间接光照
...
def singleSubScatterRender(k2,diffuse_back,map_diffuse,map_specular,map_normal,map_mask,rho_dt,render_mix_irradiance,ambientIntensity):
    ...
    return render_subscatter * map_mask
...
def createTrainFolders(root_path_object, path_views):
    path_npy = root_path_object + '/npy'
    xmkdir(path_npy)
    list_files = ['image', 'train', 'val', 'test','image_all', 'train_all', 'val_all', 'test_all']
    # list_files = ['image', 'image_all']
    for i in range(len(list_files)):
        path_all = root_path_object + '/' + list_files[i]
...
def saveTrainData(path_gene_object, path_view, input, diffuse, dtL, dtV, a_d, a_ss, a_sp, norm, mask, lightinfo, index, isCrop, size):
    '''
    input:[h,w,c], diffuse:[h,w,c], dtL:[h,w,c], dtV:[h,w,c], a_d:[h,w,c], a_ss:[h,w,c],a_sp:[h,w,c], norm:[h,w,c], mask:[h,w,c], lightinfo:[11]
    return:[]
    '''
...
def saveTrainAllData(path_gene_object, path_view, input, diffuse, dtL, dtV, a_d, a_ss, a_sp, norm, mask, lightinfo, index, isCrop, size):
    '''
    input:[h,w,c], diffuse:[h,w,c], dtL:[h,w,c], dtV:[h,w,c], a_d:[h,w,c], a_sp:[h,w,c], norm:[h,w,c], mask:[h,w,c], lightinfo:[11]
    return:[]
    '''
    path_png_all = os.path.join(path_gene_object, 'image_all')
    personName = os.path.split(path_gene_object)[1]
    image_input = input
    image_diff = diffuse
    diffuse = a_d
    ...
    savePNG(path_png_all+'/image_input/'+personName+'_'+path_view +
                f'_{str(index).zfill(4)}.png', image_input,resize=size)
    savePNG(path_png_all+'/image_diff/'+personName+'_'+path_view +
                f'_{str(index).zfill(4)}.png', image_diff,resize=size)
    savePNG(path_png_all+'/diffuse/'+personName+'_'+path_view +
                f'_{str(index).zfill(4)}.png', diffuse,resize=size)

    savePNG(path_png_all+'/sscatter_g/'+personName+'_'+path_view +
                f'_{str(index).zfill(4)}.png', sscatter_g,resize=size)
    savePNG(path_png_all+'/specular_g/'+personName+'_'+path_view +
                f'_{str(index).zfill(4)}.png', specular_g,resize=size)
    savePNG(path_png_all+'/ss_sp_dtV/'+personName+'_'+path_view +
                f'_{str(index).zfill(4)}.png', ss_sp_dtV,resize=size)
    savePNG(path_png_all+'/dtL/'+personName+'_'+path_view +
                f'_{str(index).zfill(4)}.png', dtL,resize=size)

    savePNG(path_png_all+'/normal/'+personName+'_'+path_view +
                f'_{str(index).zfill(4)}.png', normal,resize=size)
    savePNG(path_png_all+'/mask/'+personName+'_'+path_view +
                f'_{str(index).zfill(4)}.png', mask,resize=size)
    np.savetxt(path_png_all+'/lightinfo/'+personName+'_'+path_view +
